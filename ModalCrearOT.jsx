...
export default function ModalCrearOT({ onClose, onCreada }) {
  ...
  const [exito, setExito] = useState(null)   // { otNumero, waUrl, supervisorNombre }
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      // ── Éxito ──────────────────────────────────────────────────────────────
      // Generar link WhatsApp para supervisor si tiene teléfono
      let waUrl = null
      if (form.supervisor_id) {
        const sup = supervisores.find(s => s.id === form.supervisor_id)
      ...
      // Mostrar pantalla de éxito con botón WhatsApp (si hay supervisor con tel.)
      if (waUrl) {
        const sup = supervisores.find(s => s.id === form.supervisor_id)
        setExito({ otNumero, waUrl, supervisorNombre: sup ? `${sup.nombre} ${sup.apellido}`.trim() : '' })
      } else {
        // Sin supervisor/tel. → cierre inmediato
  ...
  if (exito) {
    return (
      <div style={styles.overlay}>
        <div style={{ ...styles.box, maxWidth: 460, textAlign: 'center', padding: '40px 32px' }}>
          <div style={{ fontSize: 52, marginBottom: 12 }}>✅</div>
          <h2 style={{ margin: '0 0 8px', color: 'var(--azul)', fontSize: 20 }}>
            OT {exito.otNumero} creada
          </h2>
          <p style={{ color: '#666', marginBottom: 28, fontSize: 14 }}>
            Carpetas en Drive generadas. Notifica al supervisor para que asigne el inspector.
          </p>
          ...
          <a
            ...
            style={{
              ...
              padding: '14px 24px', textDecoration: 'none', fontWeight: 700, fontSize: 16,
              marginBottom: 12,
            }}
          >
            ...
            Notificar a {exito.supervisorNombre} por WhatsApp
          </a>
