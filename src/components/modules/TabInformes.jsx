...
function SeccionCargaInforme({ ot }) {
  ...
  async function notificarSupervisor() {
    ...
    try {
      ...
      const codigos = (informesOT?.map(i => i.codigo_informe) || []).join(', ') || '(sin códigos aún)'
      ...
      let emailUrl = null
      if (sup.email) {
        const asunto = `Informes listos para revisión — OT ${ot.ot_numero}`
        const cuerpo = [
          `Hola ${sup.nombre || sup.nombre_completo},`,
          ``,
          `El inspector ${inspectorNombre} ha cargado los informes de la siguiente OT:`,
          ``,
          `OT: ${ot.ot_numero}`,
          `Cliente: ${ot.cliente || '—'}`,
          `Informe(s): ${codigos}`,
          mensajeObs ? `Observaciones: ${mensajeObs}` : null,
          ``,
          `Por favor ingresa al portal para revisar y continuar el proceso.`,
          ``,
          `— WSS División Inspección Industrial`,
        ].filter(Boolean).join('\n')
      ...
      setExito({ waUrl, emailUrl, supNombre: sup.nombre_completo })
      setMensajeObs('')
  ...
  return (
    <div style={{ marginTop: 24, border: '1px solid #E2E8F0', borderRadius: 8, overflow: 'hidden' }}>
      ...
      <div style={{ padding: '16px' }}>
        ...
        {/* Resultado de notificación — botones WA + Email */}
        {exito && (
          <div style={{ background: '#D1FAE5', border: '1px solid #6EE7B7', borderRadius: 8, padding: '12px 14px', marginBottom: 14 }}>
            <div style={{ fontSize: 13, color: '#065F46', fontWeight: 600, marginBottom: 10 }}>
              ✅ Listo — notifica a {exito.supNombre}:
            </div>
            <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
              {exito.waUrl ? (
                ...
                <span style={{ fontSize: 12, color: '#6B7280' }}>⚠️ Sin teléfono registrado</span>
              )}
              {exito.emailUrl ? (
                ...
                <span style={{ fontSize: 12, color: '#6B7280' }}>⚠️ Sin email registrado</span>
              )}
              <button onClick={() => setExito(null)} style={{ fontSize: 12, color: '#6B7280', background: 'none', border: 'none', cursor: 'pointer', padding: '8px' }}>
                Cerrar ✕
              </button>
            </div>
          </div>
